...
class FalsareeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun trackOrder(orderId: Long?) {
        ...
        if (customerId == null) {
            _alertMessage.value = "يجب تسجيل الدخول لعرض الطلب"
            return
        }
        viewModelScope.launch {
    ...
    fun login(identifier: String, password: String) {
        if (identifier.isBlank()) {
            _alertMessage.value = "أدخل رقم الهاتف أو البريد الإلكتروني"
            return
        }
        if (password.isBlank()) {
            _alertMessage.value = "أدخل كلمة المرور"
            return
        }
        viewModelScope.launch {
    ...
    fun addToCart(partner: PartnerEntity, product: ProductEntity, quantity: Int, optionsSummary: String = "") {
        ...
        if (optionsSummary.isNotEmpty()) {
            val options = _cartOptions.value.toMutableMap()
            options[product.id] = optionsSummary
            _cartOptions.value = options
        }
    ...
    fun placeOrder(
        ...
        if (partner == null || _cartItems.value.isEmpty()) {
            _alertMessage.value = "السلة فارغة"
            return
        }
    ...
    fun requestDriverPayout(amount: Double) {
        viewModelScope.launch {
            ...
            if (currentRole.value != UserRole.DRIVER || driverId == null) {
                _alertMessage.value = "لا يوجد حساب مندوب مرتبط بالجلسة الحالية"
                return@launch
            }
            if (amount <= 0) {
                _alertMessage.value = "قيمة السحب غير صالحة"
                return@launch
            }
